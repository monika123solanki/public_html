...
export const district = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['districtid'] === record['districtid']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['districtid'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['districtid'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}

                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('tasks') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/tasks/add`" class="full-width" >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/tasks"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.TasksTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/tasks/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        ...
                                                        <q-td  key="task_description" :props="props">
                                                            {{ props.row.task_description }}
                                                        </q-td>
                                                        <q-td  key="task_status" :props="props">
                                                            {{ props.row.task_status }}
                                                        </q-td>
                                                        <q-td  key="select_location" :props="props">
                                                            <q-btn v-if="props.row.select_location" @click="openPageDialog({ page: 'state/view', url: `/state/view/${props.row.select_location}` }, { closeBtn: true })" padding="xs" color="blue-1" unelevated text-color="blue" no-caps >
                                                                <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  {{ $t('state') }}
                                                            </q-btn>
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/tasks/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/tasks/edit/${props.row.id}`">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		computed: {
			...
			records: {
				get: function () {
					return this.$store.getters["tasks/records"];
				},
				set: function (value) {
					this.$store.commit("tasks/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
					return this.$store.getters["tasks/currentRecord"];
				},
				set: function (value) {
					this.$store.commit("tasks/setCurrentRecord", value);
				},
			},
		},
		meta () {
		...
		methods: {
			...mapActions("tasks", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;

                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('servers_details') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/servers_details/add`" class="full-width" >
                                <q-icon name="add"></q-icon>                                
                                {{ $t('add_new_servers_details') }} 
                            </q-btn>
                        </div>
                        <div class="col-md-auto col-12 " >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/servers_details"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.Servers_DetailsTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/servers_details/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        <q-td  key="server_name" :props="props">
                                                            {{ props.row.server_name }}
                                                        </q-td>
                                                        <q-td  key="server_ip" :props="props">
                                                            {{ props.row.server_ip }}
                                                        </q-td>
                                                        <q-td  key="machine_types" :props="props">
                                                            {{ props.row.machine_types }}
                                                        </q-td>
                                                        <q-td  key="isp_providers" :props="props">
                                                            {{ props.row.isp_providers }}
                                                        </q-td>
                                                        <q-td  key="backup_provier" :props="props">
                                                            {{ props.row.backup_provier }}
                                                        </q-td>
                                                        <q-td  key="internet_cost" :props="props">
                                                            {{ props.row.internet_cost }}
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/servers_details/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/servers_details/edit/${props.row.id}`">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'servers_detailslist',
			},
			apiPath : {
				type : String,
				default : 'servers_details/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: true,
		...
		computed: {
			pageTitle:{
				get: function () {
					...
					return this.$t('servers_details')
				}
			},
			records: {
				get: function () {
					return this.$store.getters["servers_details/records"];
				},
				set: function (value) {
					this.$store.commit("servers_details/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("servers_details", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;

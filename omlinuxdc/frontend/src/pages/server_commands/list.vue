                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('server_commands') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/server_commands/add`" class="full-width" >
                                <q-icon name="add"></q-icon>                                
                                {{ $t('add_new_server_commands') }} 
                            </q-btn>
                        </div>
                        <div class="col-md-auto col-12 " >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/server_commands"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.Server_CommandsTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/server_commands/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        <q-td  key="commands_name" :props="props">
                                                            {{ props.row.commands_name }}
                                                        </q-td>
                                                        <q-td  key="command_actions" :props="props">
                                                            {{ props.row.command_actions }}
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/server_commands/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/server_commands/edit/${props.row.id}`">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'server_commandslist',
			},
			apiPath : {
				type : String,
				default : 'server_commands/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: true,
		...
		computed: {
			pageTitle:{
				get: function () {
					...
					return this.$t('server_commands')
				}
			},
			records: {
				get: function () {
					return this.$store.getters["server_commands/records"];
				},
				set: function (value) {
					this.$store.commit("server_commands/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("server_commands", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;

                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('power_management') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/power_management/add`" class="full-width" >
                                <q-icon name="add"></q-icon>                                
                                {{ $t('add_new_power_management') }} 
                            </q-btn>
                        </div>
                        <div class="col-md-auto col-12 " >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/power_management"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.Power_ManagementTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/power_management/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        <q-td  key="bettry" :props="props">
                                                            {{ props.row.bettry }}
                                                        </q-td>
                                                        <q-td  key="inverter" :props="props">
                                                            {{ props.row.inverter }}
                                                        </q-td>
                                                        <q-td  key="power_consumes" :props="props">
                                                            {{ props.row.power_consumes }}
                                                        </q-td>
                                                        <q-td  key="backup_hours" :props="props">
                                                            {{ props.row.backup_hours }}
                                                        </q-td>
                                                        <q-td  key="estimated_bill_month" :props="props">
                                                            {{ props.row.estimated_bill_month }}
                                                        </q-td>
                                                        <q-td  key="estimated_cost_per_day" :props="props">
                                                            {{ props.row.estimated_cost_per_day }}
                                                        </q-td>
                                                        <q-td  key="estimated_cost_month" :props="props">
                                                            {{ props.row.estimated_cost_month }}
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/power_management/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/power_management/edit/${props.row.id}`">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'power_managementlist',
			},
			apiPath : {
				type : String,
				default : 'power_management/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: true,
		...
		computed: {
			pageTitle:{
				get: function () {
					...
					return this.$t('power_management')
				}
			},
			records: {
				get: function () {
					return this.$store.getters["power_management/records"];
				},
				set: function (value) {
					this.$store.commit("power_management/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("power_management", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;

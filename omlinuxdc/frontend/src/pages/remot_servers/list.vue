                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('remot_servers') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/remot_servers/add`" class="full-width" >
                                <q-icon name="add"></q-icon>                                
                                {{ $t('add_new_remot_servers') }} 
                            </q-btn>
                        </div>
                        <div class="col-md-auto col-12 " >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/remot_servers"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.Remot_ServersTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/remot_servers/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                                        </q-td>
                                                        ...
                                                        <q-td  key="add_servers" :props="props">
                                                            {{ props.row.add_servers }}
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/remot_servers/view/${props.row.id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/remot_servers/edit/${props.row.id}`">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'remot_serverslist',
			},
			apiPath : {
				type : String,
				default : 'remot_servers/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: true,
		...
		computed: {
			pageTitle:{
				get: function () {
					...
					return this.$t('remot_servers')
				}
			},
			records: {
				get: function () {
					return this.$store.getters["remot_servers/records"];
				},
				set: function (value) {
					this.$store.commit("remot_servers/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("remot_servers", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
